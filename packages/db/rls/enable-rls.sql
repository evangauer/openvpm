...
DECLARE
  ...
  tbls text[] := array[
    ...
    'subscription_cadence_operations','subscription_checkout_attempts','treatment_plans','treatment_templates','usage_records','users','vaccination_records',
...
REVOKE ALL ON subscription_cadence_operations FROM openpims_app;
GRANT SELECT, INSERT, UPDATE ON subscription_cadence_operations TO openpims_app;
...
-- Backup sweep evidence is global operational state. The app role may append
-- and inspect evidence only while an explicit system transaction is active;
-- completed evidence is immutable to that role.
