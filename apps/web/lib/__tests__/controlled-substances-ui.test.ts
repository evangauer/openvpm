...
describe("controlled substances UI bounds", () => {
  ...
  it("keeps log entry controls aligned to shared policy", () => {
    ...
    expect(isControlledSubstanceRequiredTextInputValid(" Ketamine ", 255)).toBe(
      true,
    );
  ...
  it("fails closed when patient or witness lookups return no payload", () => {
    ...
    expect(source).toContain("Patient lookup is unavailable. Please retry.");
    expect(source).toContain("Witness lookup is unavailable. Please retry.");
    expect(
      source.indexOf("patientsQuery.error || patientsMissing"),
    ).toBeLessThan(source.indexOf(': "No patient"'));
    expect(
  ...
  it("renders log timestamps in the practice timezone", () => {
    expect(source).toContain("trpc.controlledSubstances.settings.useQuery");
    ...
    expect(source).toContain("{canRecordControlledSubstance && showForm && (");
    ...
    expect(source).not.toContain(
      "const logTimeZone = settingsQuery.data?.timezone",
    );
  ...
  it("gates direct-route access to controlled-substance roles", () => {
    ...
    expect(
      source.indexOf("function ControlledSubstancesLogPage()"),
    ).toBeLessThan(
