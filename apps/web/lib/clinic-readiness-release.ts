...
export function evaluateClinicReadinessRelease(
  ...
  if (root?.evidenceFormatVersion !== 1) {
    reasons.push("Clinic readiness evidence format version must be 1.");
  }
  const releaseSha =
    root && typeof root.releaseSha === "string" && SHA_PATTERN.test(root.releaseSha)
      ? root.releaseSha.toLowerCase()
      : null;
  if (!releaseSha) reasons.push("Release SHA must be an exact 40-character commit.");
  ...
  if (!restore) {
    ...
    if (restore.synthetic !== false) {
      reasons.push("A provider-backed non-synthetic restore drill is required.");
    }
    ...
    if (
      ...
      !["schedulingRows", "clinicalRows", "invoiceRows", "paymentRows", "fileAccessRows"].every(
        (name) => typeof smoke?.[name] === "number" && smoke[name] > 0,
      )
    ) {
      reasons.push("Post-restore clinic workflow smoke evidence is incomplete.");
    }
  }
