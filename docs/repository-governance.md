...
through GitHub immediately before a release decision. `Staging` and `Production`
must disable administrator bypass, prevent self-review, name at least one
required reviewer, and accept deployments from `staging` and `main`
respectively. Both canonical release branches must enforce protection for
administrators, require code-owner and last-push review, dismiss stale approvals,
require conversation resolution and strict current-head checks, and prohibit
force pushes and deletion. `staging` requires at least one approval; `main`
requires two. The required check set includes the full build, golden clinic
